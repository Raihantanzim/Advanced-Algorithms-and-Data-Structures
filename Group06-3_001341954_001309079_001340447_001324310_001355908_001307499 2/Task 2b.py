...
def main():
    ...
    # Count occurrences of each number of stops
    max_stops = max(journey_counts)
    stop_counts = [0] * (max_stops + 1)
    for count in journey_counts:
        stop_counts[count] += 1

    # Plot a line graph of journey counts.
    plt.plot(range(len(stop_counts)), stop_counts, marker='o', label='Journey Distribution')
    plt.title('Number of Stops vs. Station Pairs')
